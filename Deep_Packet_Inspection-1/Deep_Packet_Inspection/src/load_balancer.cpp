...
void LoadBalancer::run() {
    while (running_) {
        ...
        if (fp_index >= 0 && fp_index < (int)fp_queues_.size()) {
            fp_queues_[fp_index]->push(std::move(*job_opt));
            packets_dispatched_++;
            if (fp_index < (int)per_fp_counts_.size()) {
...
LoadBalancer::LBStats LoadBalancer::getStats() const {
    ...
    // Safe copy - only copy if sizes match
    stats.per_fp_packets.resize(num_fps_, 0);
    for (int i = 0; i < num_fps_ && i < (int)per_fp_counts_.size(); i++) {
        stats.per_fp_packets[i] = per_fp_counts_[i];
    }
